...
local luaxp = require('scripts.BardicOverhaul.util.luaxp')
...
local uiTextures = {
    ...
    pianoRollNote = ui.texture {
        path = 'textures/BardicOverhaul/ui/pianoroll-note.dds',
    }
}
...
local function stopAllSounds()
    ...
    for _, profile in pairs(profiles) do
        for j = 0, 127 do
            local filePath = 'sound\\BardicOverhaul\\samples\\' .. profile.name .. '\\' .. profile.name .. '_' .. Song.noteNumberToName(j) .. '.wav'
...
getSongManager = function()
    ...
    Editor.songs = storage.playerSection('BardicOverhaul'):getCopy('songs/preset') or {}
    local songs = {}
...
function Editor:onToggle()
    if self.active then
        ...
        core.sendGlobalEvent('BO_EditorUnpause')
    else
        self:createUI()
        self.active = true
        I.UI.setMode(I.UI.MODE.Interface, {windows = {}})
        core.sendGlobalEvent('BO_EditorPause')
    end
end
