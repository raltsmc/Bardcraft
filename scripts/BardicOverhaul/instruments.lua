...
local Instruments = {
    Lute = {
        ...
        eventHandler = function(data)
            if data.type == 'NoteEvent' then
                ...
                local doubleStrum = lastTime and data.time - lastTime < 0.05
    ...
    Ocarina = {
        ...
        eventHandler = function(data)
            if data.type ~= 'NoteEvent' then return end
            if data.note == data.lastNote then return end -- Only change fingering if the note changes
            ...
            local animIndex = 0
            if data.lastNote and math.abs(data.note - data.lastNote) == 5 then
                if data.note > data.lastNote then animIndex = animIndex + 1
                elseif data.note < data.lastNote then animIndex = animIndex - 1 end
            end
