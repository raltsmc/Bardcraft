...
return {
    engineHandlers = {
        onInit = function()
            local midi, err = MIDI.parseMidiFile('test1.mid')
            if not midi then
                print("Error parsing MIDI file: " .. err)
            else
                for i, track in ipairs(midi.tracks) do
                    print("Track", i)
                    for _, event in ipairs(track) do
                        print(event.time, event.type, event.note or "", event.program or "")
                    end
                end
            end
        end,
